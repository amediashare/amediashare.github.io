...
function ffmpeg(opts, cb, pre) {
    function waitResult(checkFunc, ms, progress){
        return new Promise((resolve) => {
            ...
            let interval = setInterval(()=> {
                ...
                if ((result) || (i * 100> ms)){
                    ...
                    progress ? progress( i* 100 / ms) : null;
                    i++;
                }
            }, 100);
        });
	...
	if (!isNode) {
		...
		Module["preRun"] = (function () {
			if (Module["files"]) {
				...
				if (Module['OutputfileSystem']){
					let outputFilePath = Module["arguments"][Module["arguments"].length - 1];
					FS.mount(Module['OutputfileSystem'], {
						files: [outputFilePath.replace(Module["outputDirectory"] + '/' , '')]
			...
			if (result && result.object && result.object.contents) {
				for (var i in result.object.contents) {
					if (result.object.contents.hasOwnProperty(i)) {
						if (Module['OutputfileSystem']){
							//result.object.contents[i].contents.close(result.object.contents[i]);
							buffers.push({
								name: result.object.contents[i].name
							});
						}else{						
							buffers.push({
								name: i,
	...
	function integrateWasmJS() {
		var wasmTextFile = "core.wast";
		var wasmBinaryFile = "core.wasm";
		var asmjsCodeFile = "core.temp.asm.js";
		...
		function doNativeWasm(global, env, providedBuffer) {
			...
			if (Module["instantiateWasm"]) {
				try {
					return Module["instantiateWasm"](wasmBinaryFile, info, receiveInstance)
				} catch (e) {
	...
	STATICTOP = STATIC_BASE + 13333488;
	__ATINIT__.push({
		func: (function () {
			___emscripten_environ_constructor()
		})
	});
	var STATIC_BUMP = 13333488;
		...
		default_tty1_ops: {
			put_char: (function (tty, val) {
				if (val === null || val === 10 || val === 13) {
					...
					if (tty.output.length >= 84){
						str = UTF8ArrayToString(tty.output, 0);
						if (/frame.+\/s?/.exec(str)){
							Module["printErr"](str);
							tty.output = []				
						}						
	...
	var WORKERFS = {
		...
		createNode: (function (parent, name, mode, dev, contents, mtime) {
			...
			if (mode === WORKERFS.FILE_MODE) {
				node.size = contents.data.size;
				node.contents = contents.data
			} else {
				node.size = 4096;
				node.contents = {}
			}
		...
		node_ops: {
			...
			lookup: (function (parent, name) {
				return parent.contents[Object.keys(parent.contents)[0]].contents[name]
	...
	function nullFunc_fiiiiif(x) {
		err("Invalid function pointer called with signature 'fiiiiif'. Perhaps this is an invalid value (e.g. caused by calling a virtual method on a NULL pointer)? Or calling a function with an incorrect type, which will fail? (it is worth building your source files with -Werror (warnings are errors), as warnings can indicate undefined behavior which can cause this)");
		err("Build with ASSERTIONS=2 for more info.");
		abort(x)
	}
	...
	function nullFunc_iiiiiiiiiiiiiiiii(x) {
		err("Invalid function pointer called with signature 'iiiiiiiiiiiiiiiii'. Perhaps this is an invalid value (e.g. caused by calling a virtual method on a NULL pointer)? Or calling a function with an incorrect type, which will fail? (it is worth building your source files with -Werror (warnings are errors), as warnings can indicate undefined behavior which can cause this)");
		err("Build with ASSERTIONS=2 for more info.");
		abort(x)
	}
	...
	function nullFunc_iiiijiiiiii(x) {
		err("Invalid function pointer called with signature 'iiiijiiiiii'. Perhaps this is an invalid value (e.g. caused by calling a virtual method on a NULL pointer)? Or calling a function with an incorrect type, which will fail? (it is worth building your source files with -Werror (warnings are errors), as warnings can indicate undefined behavior which can cause this)");
		err("Build with ASSERTIONS=2 for more info.");
		abort(x)
	}
	function nullFunc_iiiijj(x) {
		err("Invalid function pointer called with signature 'iiiijj'. Perhaps this is an invalid value (e.g. caused by calling a virtual method on a NULL pointer)? Or calling a function with an incorrect type, which will fail? (it is worth building your source files with -Werror (warnings are errors), as warnings can indicate undefined behavior which can cause this)");
		err("Build with ASSERTIONS=2 for more info.");
		abort(x)
	}
	...
	function nullFunc_viiiiiiiiddddd(x) {
		err("Invalid function pointer called with signature 'viiiiiiiiddddd'. Perhaps this is an invalid value (e.g. caused by calling a virtual method on a NULL pointer)? Or calling a function with an incorrect type, which will fail? (it is worth building your source files with -Werror (warnings are errors), as warnings can indicate undefined behavior which can cause this)");
		err("Build with ASSERTIONS=2 for more info.");
		abort(x)
	}
	...
	Module["wasmTableSize"] = 14871;
	Module["wasmMaxTableSize"] = 14871;
	...
	Module.asmLibraryArg = {
		...
		"nullFunc_fiiiiif": nullFunc_fiiiiif,
		...
		"nullFunc_iiiijiiiiii": nullFunc_iiiijiiiiii,
		...
		"nullFunc_viiiiiiiiddddd": nullFunc_viiiiiiiiddddd,
	...
	this.WaitDependAndRun = async function(){
		if (await waitResult(()=> {return runDependencies==0}, 10 * 60 * 1000)){
